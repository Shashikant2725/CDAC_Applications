...
const Script = (props) => {
  ...
  function stopScript() {
    ...
    //   let platform = localStorage.getItem("Platform");
    //   let version = localStorage.getItem("Version");
    //   dispatch(stopNetwork(platform, version,mail));
    // }
  }
    else {
      // setStopTick(true)
  ...
  function removeScript() {
    ...
    //   let platform = localStorage.getItem("Platform");
    //   let version = localStorage.getItem("Version");
    //   dispatch(removeNetwork(platform, version, removestartTime,mail));
    // }
    }
    else {
      // setRemoveTick(true)
  ...
  function generateScript(e) {
    ...
    if (imageResponse !== undefined) {
      ...
      if (appresponse[1].function === null) {
        console.log('gS2',appresponse )
        
        let genScriptTime = Date.now();
        localStorage.setItem("genScriptStartTime", genScriptTime)
        const organisation = JSON.parse(appresponse[0].Organisation);
        isDisplay = false
        ...
        let platform = appresponse[0].platform
        let version = appresponse[0].verison
        let environment = appresponse[0].environment
        let channel = appresponse[0].channel
        let orderers = appresponse[0].Orderers.split(",");
        let pos = appresponse[0].checked;
        dispatch(genScript(platform, environment, version, channel, orderers, cas, peers, pos, db,dynamicToken,mail));
        console.log("pos", pos)
        console.log("db is", db)
        // return genTickButton
      }
      // if (appresponse[1].function === null)  {
      //   console.log("outside if condition  1")
      //   let genScriptTime = Date.now();
      //   localStorage.setItem("genScriptStartTime", genScriptTime)
      //   const organisation = JSON.parse(localStorage.getItem("Organisation"));
      //   isDisplay = false
      ...
      //   let orderers = localStorage.getItem("Orderers").split(",");
      //   let pos = localStorage.getItem("checked");
      //   dispatch(genScript(platform, environment, version, channel, orderers, cas, peers, pos, db,mail));
      //   console.log("pos", pos)
      //   console.log("db is", db)
      // }
    }
      ...
      <div className="list-group" style={{ width: '100%', height: '100%' }}>
        <div className="side-menu" style={{ width: '15%', height: '80%' }}>
          <nav className="nav-style">
            ...
            <Link to="/generateScript" onClick={generateScript} className="d-flex align-items-center check12" >
              {/*<a onClick={generateScript} className="d-flex flex-row align-items-center">*/}
              <AiFillSetting className="" style={{ marginRight: "5px", fontSize: "18px" }} />  Generate Script
            </Link>
