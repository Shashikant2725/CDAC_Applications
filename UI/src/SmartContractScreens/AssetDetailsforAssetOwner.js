...
import TableNew from "./TableExample"
...
const Tables = () => {
  ...
  const onInputChangefornewOrg = e => {
    setnewOrgName({ ...newOrgName, [e.target.name]: e.target.value });
  };
  ...
  const handleChange1 = async (event) => {
    ...
    for (var i = 0; i < DomainIds.length; i++) {
        let domains2 = '';
        await axios.get('http://10.244.3.33:7000/transferGenericAsset/' + DomainIds[i]).
            then(response => {
...
const handleChange2=async(event) =>{
  ...
  //Edit data
  const handleEditPostForm = (e, orgName,userName) => {
    e.preventDefault()
...
console.log("Edit Save Data:",e);
    ...
    const article = {
        ...
        Asset_Id:editPostId,
        newOrgName:editFormData.newOrgName,
        newOwnerName:editFormData.newOwnerName
       
     
     };
     console.log("Article Object::",article);
     const transferOwner = "http://10.244.3.187:4000/api/v1/transferOwner";
     const ownerDetail = await axios.post(transferOwner,article).then((res)=>toast.success("Owner Updated Successfully!!"))
     console.log("this is updated ownerDetail::",ownerDetail)
...
//      window.setTimeout(function(){window.location.reload('http://10.244.3.187:3000/addstakeholder')},3000)  
  ...
  const fetchUrl = "http://10.244.2.142:8888/queryAllDetails";
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      setPosts(JSON.parse(data.data.Result))

      console.log("getAllAssets::",JSON.parse(data.data.Result));
...
alert("success");
  ...
  return (
    ...
    <div>
      ...
      <table className="table table-bordered table-responsive" style={{marginTop:"4%",width:"100%",height:"100%"}}>
        <thead>
          <tr>
          <th scope="col">Asset Id</th>
          <th scope="col">Public_Mandatory</th>
            <th scope="col">Private_Mandatory</th>
            <th scope="col">Public_Mutable</th>
            <th scope="col">Private_Mutable</th>
      ...
      <div className="modal fade" id="editModalForm" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-header">
            <h5 className="modal-title" id="exampleModalLabel">Transfer Ownership</h5>
            ...
            <div className="modal-body">
            {/* <div className='container'>Org</div>
                <div className='container'>User</div>
                <div className='container'>newOwnerOrg</div>
                <div className='container'>newOwner</div> */}
              <form onSubmit={handleFormSave}>
               ...
               {/* <label className="form-label">Select Domain</label>
                ...
                <div className="mb-3">
                  <label className="form-label">new Owner Org</label>
                  <input
                   
                    className="form-control"
                    placeholder='new Owner Org'
                    name="newOrgName"
                    value={editFormData.newOrgName}
                    required
                    onChange={handleEditFormClick("newOrgName")}
                  ></input>
                </div>

                <div className="mb-3">
                  <label className="form-label">newOwner</label>
                  <input
                   
                    className="form-control"
                    placeholder='new Owner Name'
                    name="newOwnerName"
                    value={editFormData.newOwnerName}
                    required
                    onChange={handleEditFormClick("newOwnerName")}
                  ></input>
                </div>
...
{/* <Button  type="submit" startIcon={<SaveAltIcon />}
              data-bs-dismiss="modal"
              className="btn btn-success float-end"
