...
//  this.hideComponent("showHideLName")
 ...
 onGetHistory=async()=>{
  ...
  await axios.post('http://10.244.3.33:5000/historyCode', {
    Code: 
...
onchooseDesignPattern=()=> {
 ...
 onEncryption=async()=>{
  ...
  await axios.post('http://10.244.3.33:5000/encryptionCode', {
...
onDownload=()=>{
  ...
  axios.post(`http://10.244.3.33:5000/test`,functions);
  alert('Chaincode Downloaded Successfully');

  window.location.href='http://10.244.3.33:5000/Downloads';
  // this.toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Chaincode Downloaded Successfully', life: 3000 });

  window.setTimeout(function(){window.location.reload('10.244.3.33:3001/templateStudio')},2000)  
}
...
await axios.post('http://10.244.3.33:5000/pdc',Json);
  alert('All data has been saved suceessfully click to download file');
  // this.handleClosePDCA();
  // window.location.reload('10.244.3.33:300/templateStudio')
}
...
componentDidMount() {
    ...
    console.log("Bhai Countery Id::",e.target.value)
        ...
        axios.get('http://10.244.3.33:4300/api/v1/domain/' + e.target.value).then(response => {
    ...
    axios.get('http://10.244.3.33:4300/api/v1/stakeholder/' + e.target.value).then(response => {
  ...
  temp=this.state.CityData[0].FunctionName;
    ...
    handleSubmit=async(event)=>{
      ...
      toast.success(`Your Server Started on:${this.state.IP}:${this.state.PORT}`)
      window.setTimeout(function(){window.location.reload('10.244.3.187:2000/restapi')},2000)  
  ...
  const {
    ...
    <div class="col-sm">
       ...
       <FormControl fullWidth >
        ...
        >
        {/* <input type="text" style={{margin:"15px",marginLeft:"7px"}} className="form-control" required/> */}
    ...
    <div class="col-sm">
    <FormControl fullWidth >
        {/* <InputLabel id="demo1-simple-select-label" style={{ marginTop: '15px',fontSize:"18px" }}>click on this IP</InputLabel> */}
        
        <Form.Label>Enter IP</Form.Label>
        <Form.Control type="float" style={{ marginTop: '1px',height:'55px'}} onChange={(e)=>this.setState({IP:e.target.value})} placeholder="Enter Resource IP Address"  name="IP"/>
        {/* <Form.Control type="float" style={{ marginTop: '1px',height:'55px'}} placeholder="Enter this IP" /><br /> */}
    ...
    <FormControl fullWidth >
        ...
        <Form.Label>Enter Port</Form.Label>
        <Form.Control type="number" style={{ marginTop: '1px',height:'55px'}} onChange={(e)=>this.setState({PORT:e.target.value})} placeholder="Enter Port Number" />
        {/* <Form.Control type="float" style={{ marginTop: '1px',height:'55px'}} placeholder="Enter this IP" /><br /> */}
...
<div>
  {/* <button onclick="myFunction()">Upload</button>
    ...
    <button type="submit"   className="btn btn-primary" style={{ height:"35px",textAlign:"justify", backgroundColor: '#137EA9', border: 'none',marginLeft:"0.2%",marginTop:"-1%",marginBottom:"2%"}}>Start Rest API Server</button>
