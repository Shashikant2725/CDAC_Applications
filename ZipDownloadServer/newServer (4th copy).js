...
app.post('/test', async function (req, res, next) {
            ...
            try{
              console.log("hello from test")
                ...
                //  datas.push(req.body);
                  // datawithAsset.push(req.body.user.asset);
                  // nums.includes(3
                  assetrequire = datas.includes(true);  
                // datas=req.body;
                console.log("datas::",datas);
                // console.log("datawithAsset::",datawithAsset);
console.log('data require ',assetrequire)
                ...
                let res = await axios.get(`http://10.244.3.187:4300/api/v1/nodefunction/${datas[0]}`);
                  mongodata = res.data;
                  console.log("mongoData::", res.data)
 ...
 module.exports = ${Domainname};
        ...
        if(getEncryptionPack!=1)
              ...
              { 
                ...
                class ${Domainname} extends Contract {
               `               
                fspromises.appendFile(path.join(__dirname,"Chaincode",Domainname,"node",CCfilename),chaincodeheader1);
            ...
            for(var i=0;i<datas.length;i++){
             ...
             foo(i);
              ...
              async function foo(i){
                let ccbodyrequest = await axios.get(`http://10.244.3.187:4300/api/v1/nodefunction/${datas[i]}`);
                ...
                console.log('Chaincode Body for '+datas[i]+' Inegrated Successfully');  
                 ...
                 if(i<1){
                  ...
                  if(getHistoryFlag==1)
                  {
                     fspromises.appendFile((path.join(__dirname,"Chaincode",Domainname,"node",CCfilename)),getHistoryCode.toString());
                     await fspromises.appendFile((path.join(__dirname,"Chaincode",Domainname,"node",CCfilename)),'\n');
                     getHistoryFlag=0;
                  ...
                  if(getEncryptFlag==1)
                  {
                     fspromises.appendFile((path.join(__dirname,"Chaincode",Domainname,"node",CCfilename)),getEncryptionCode.toString());
                     await fspromises.appendFile((path.join(__dirname,"Chaincode",Domainname,"node",CCfilename)),'\n');
                     getEncryptFlag=0;
                  ...
                  if(getPDCFile==1){
                    ...
                    fsmove1.move(path.join(__dirname,oldPath), path.join(__dirname,newPath), function (err) {
                      if (err) throw err
                ...
                function foo1(){
                  console.log("i am inside asset Condition")

                  const zip = new AdmZip();
                  try {
                    if(assetrequire == true){
                       zip.addLocalFolder(path.join(__dirname,"Chaincode"))
                      ...
                      try {
                        ...
                        fs1.copySync(path.join(__dirname,"Chaincode",Domainname), path.join(__dirname,"Chaincode1","Chaincode"), { overwrite: true|false })
                        console.log('success!')
                      } catch (err) {
                        console.error(err)
                      }
                       zip.addLocalFolder(path.join(__dirname,"Chaincode1"));
...
app.use(function(req, res, next) {
 ...
 app.get('/Downloads',async function(req,res){
  ...
  const Result=({requestsLastMinute: cnt});
  console.log("Result::",Result)
   res.download(path.join(__dirname,'zipcontents',domainFile),Result);

  // await res.json(Result);
})


app.post('/historyCode',async function(req,res){
 var datas=req.body;
console.log("getHistoryCode::",datas.Code);
getHistoryCode=datas.Code;
console.log("Objetc to String::",getHistoryCode);
getHistoryFlag=1
res.send({message:"Data Has Been Saved Successfully"}).status(200)
})

app.post('/encryptionCode',async function(req,res){
  var datas=req.body;
 console.log("getEncryptionCode::",datas.Code);
 getEncryptionPackageCode=datas.Package;
 console.log("getEncryptionPackageCode::",getEncryptionPackageCode)
 getEncryptionCode=datas.Code;
 console.log("Objetc to String::",getEncryptionCode);
 getEncryptionPack=1;
 getEncryptFlag=1;
 res.send({message:"Data Has Been Saved Successfully"}).status(200)
...
/////////////////////////PDCA JSON File Creation//////////////////////////
app.post('/pdc', async function (req, res) {
  console.log(req.body)
  var obj = req.body
  // getPDCFile=obj;
  // console.log("getPDCFile:",getPDCFile);
  getPDCFile=1;
  for (var i = 0;i<Object.keys(obj).length;i++){
      obj[i].requiredPeerCount = '0'
      obj[i].maxPeerCount = '3'
      obj[i].blockToLive = '1000000'
      obj[i].memberOnlyRead = true,
      obj[i].memberOnlyWrite = true
  }
  console.log('Object Display',util.inspect(obj, {depth: null}));
  fs.writeFile("collection_config.json", JSON.stringify(obj, null, 4), 'utf8', function (err) {
      if (err) {
      ...
      console.log('Chaincode Body for '+datas[i]+' Inegrated Successfully');  
       ...
       fsmove.move(path.join(__dirname,"Chaincode",Domainname,"node",CCfilename), path.join(__dirname,"Chaincode",Domainname,"node",'lib',CCfilename), (err) => {
        ...
        async function foo1(){
          const zip = new AdmZip();
          if(assetrequire == true){
            await zip.addLocalFolder(path.join(__dirname,"Chaincode-poe"))
            // await zip.addLocalFolder(path.join(__dirname,"Chaincode",));
            ...
            try {
              ...
              fs1.copySync(path.join(__dirname,"Chaincode",Domainname), path.join(__dirname,"ChaincodePOE","Chaincode"), { overwrite: true|false })
              console.log('success!')
            } catch (err) {
              console.error(err)
            }
            await zip.addLocalFolder(path.join(__dirname,"ChaincodePOE"));
...
fspromises.appendFile((path.join(__dirname,"Chaincode",Domainname,"node",'processor.js')),processor);
      console.log('index.js Inegrated Successfully');
      ...
      console.log('Chaincode Body for '+datas[i]+' Inegrated Successfully');  
       ...
       fsmove.move(path.join(__dirname,"Chaincode",Domainname,"node",CCfilename), path.join(__dirname,"Chaincode",Domainname,"node",'lib',CCfilename), (err) => {
        ...
        async function foo1(){
          const zip = new AdmZip();
          if(assetrequire == true){
            await zip.addLocalFolder(path.join(__dirname,"ChaincodeInsuranceAsset"))
            // await zip.addLocalFolder(path.join(__dirname,"Chaincode",));
            ...
            try {
              ...
              await fs1.copySync(path.join(__dirname,"Chaincode",Domainname), path.join(__dirname,"ChaincodeInsurance","Chaincode"), { overwrite: true|false })
              console.log('success!')
            } catch (err) {
              console.error(err)
            }
            await zip.addLocalFolder(path.join(__dirname,"ChaincodeInsurance"));
